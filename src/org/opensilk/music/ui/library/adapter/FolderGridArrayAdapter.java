...
public class FolderGridArrayAdapter extends AbsEndlessGridArrayAdapter {
    ...
    protected static class ResultCallback extends Result.Stub {
        private final WeakReference<FolderGridArrayAdapter> adapter;

        protected ResultCallback(FolderGridArrayAdapter adapter) {
            this.adapter = new WeakReference<>(adapter);
        }

        @Override
        public void success(final List<Bundle> items, final Bundle paginationBundle) throws RemoteException {
            final FolderGridArrayAdapter a = adapter.get();
            if (a != null) {
