...
//                                R.menu.popup_delete,
    ...
    public boolean onItemClicked(Context context, OverflowAction action, Bundleable item) {
        ...
        switch (action) {
            case PLAY_ALL:
                playbackController.playTracksFrom(tc.tracksUri, 0, sortOrder);
                return true;
            case SHUFFLE_ALL:
                playbackController.shuffleTracksFrom(tc.tracksUri);
                return true;
            case ADD_TO_QUEUE:
                playbackController.addTracksToQueueFrom(tc.tracksUri, sortOrder);
                return true;
