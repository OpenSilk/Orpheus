...
public class PlaybackControllerImpl implements PlaybackController {
    ...
    @Inject
    public PlaybackControllerImpl(@ForApplication Context mAppContext) {
    ...
    public void playorPause() {
        sendCustomAction(PlaybackConstants.ACTION.TOGGLE_PLAYBACK, null);
    }
    ...
    public void enqueueAll(List<Uri> queue, int where) {
        sendCustomAction(PlaybackConstants.ACTION.ENQUEUE, BundleHelper.b().putList(clampList(queue, 0)).putInt(where).get());
    }
    ...
    public void playAll(List<Uri> list, int startpos) {
        sendCustomAction(PlaybackConstants.ACTION.PLAY_ALL, BundleHelper.b().putList(clampList(list, startpos)).putInt(startpos).get());
    }

    public void removeQueueItem(Uri uri) {
        sendCustomAction(PlaybackConstants.ACTION.REMOVE_QUEUE_ITEM, BundleHelper.b().putUri(uri).get());
    }

    public void removeQueueItemAt(int pos) {
        sendCustomAction(PlaybackConstants.ACTION.REMOVE_QUEUE_ITEM_AT, BundleHelper.b().putInt(pos).get());
    }

    public void clearQueue() {
        sendCustomAction(PlaybackConstants.ACTION.CLEAR_QUEUE, null);
    }

    public void moveQueueItemTo(Uri uri, int newPos) {
        sendCustomAction(PlaybackConstants.ACTION.MOVE_QUEUE_ITEM_TO, BundleHelper.b().putUri(uri).putInt(newPos).get());
    }

    public void moveQueueItem(int from, int to) {
        sendCustomAction(PlaybackConstants.ACTION.MOVE_QUEUE_ITEM, BundleHelper.b().putInt(from).putInt2(to).get());
    }

    public void moveQueueItemToNext(int pos) {
        sendCustomAction(PlaybackConstants.ACTION.MOVE_QUEUE_ITEM_TO_NEXT, BundleHelper.b().putInt(pos).get());
    }

    @DebugLog
    public void switchToNewRenderer(@Nullable ComponentName componentName) {
        if (hasController()) {
            mImpl.getMediaController().sendCommand(PlaybackConstants.CMD.SWITCH_TO_NEW_RENDERER,
    ...
    public void getCurrentRenderer(final Action1<ComponentName> onNext) {
        if (hasController()) {
            mImpl.getMediaController().sendCommand(PlaybackConstants.CMD.GET_CURRENT_RENDERER, null,
    ...
    private void fetchRepeatMode() {
        if (hasController()) {
            mImpl.getMediaController().sendCommand(PlaybackConstants.CMD.REQUEST_REPEATMODE_UPDATE, null, new ResultReceiver(mCallbackHandler) {
                @Override
                protected void onReceiveResult(int resultCode, Bundle resultData) {
    ...
    private void fetchShuffleMode() {
        if (hasController()) {
            mImpl.getMediaController().sendCommand(PlaybackConstants.CMD.REQUEST_SHUFFLEMODE_UPDATE, null, new ResultReceiver(mCallbackHandler) {
                @Override
                protected void onReceiveResult(int resultCode, Bundle resultData) {
    ...
    private void fetchAudioSessionId() {
        if (hasController()) {
            mImpl.getMediaController().sendCommand(PlaybackConstants.CMD.REQUEST_AUDIOSESSION_ID, null, new ResultReceiver(mCallbackHandler) {
                @Override
                protected void onReceiveResult(int resultCode, Bundle resultData) {
